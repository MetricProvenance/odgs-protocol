...
def _load_standard_metrics_context() -> str:
    ...
    path = settings.PROJECT_ROOT / "lib" / "schemas" / "legislative" / "standard_metrics.json"
    if not path.exists():
...
def _load_rules_context() -> str:
    ...
    path = settings.PROJECT_ROOT / "lib" / "schemas" / "legislative" / "standard_data_rules.json"
    if not path.exists():
        return ""
    try:
        data = json.loads(path.read_text())
...
def _load_dq_dimensions_context() -> str:
    ...
    path = settings.PROJECT_ROOT / "lib" / "schemas" / "judiciary" / "standard_dq_dimensions.json"
    if not path.exists():
        return ""
    try:
        data = json.loads(path.read_text())
...
def _load_business_processes_context() -> str:
    ...
    path = settings.PROJECT_ROOT / "lib" / "schemas" / "executive" / "business_process_maps.json"
    if not path.exists():
        return ""
    try:
        data = json.loads(path.read_text())
